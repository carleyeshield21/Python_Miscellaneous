import cv2
import time
from camera_main_email_screenshot import send_email
import glob
import os
...
while True:
    ...
    if  status_list[0] == 1 and status_list[1] == 0:
        email_thread = Thread(target=send_email, args=(photos_detected[photo_index], ))
        email_thread.daemon = True
        email_thread.start()

        # send_email(photos_detected[photo_index])
        # photos_detected = glob.glob('detected_images/*.png')
        # photo_index = int(len(photos_detected) / 2)
        # print(photo_index)
